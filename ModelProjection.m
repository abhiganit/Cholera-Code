function [Yt,Pt]= ModelProjection(beta,WI,tA,DB,DA,DAE,Ctv,K,n,Rtv,RIF,rl,RF,rh,tau,maxtau,CF,P,RC,H,NWP)
...
[Yt,~,~,~,~,~,~]= LogisticModel(beta,WI,tA(:,1:length(WI(1,:))),DB,DA,DAE,Ctv(:,1:length(WI(1,:))),K,n,Rtv(:,1:length(WI(1,:))),RIF,rl,RF,rh,tau,maxtau,CF,P,RC,H);
...
for ii=1:NWP % Loop through the number of weeks that are to be projected
    ...
    [temp2,~,~,~,~,~,~]= LogisticModel(beta,WT,tA(:,1:length(WT(1,:))),DB,DA,DAE,Ctv(:,1:length(WT(1,:))),K,n,Rtv(:,1:length(WT(1,:))),RIF,rl,RF,rh,tau,maxtau,CF,P,RC,H); % Run model with appendend data
