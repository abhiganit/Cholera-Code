close all;
clear;
[WI2,~,tA2,Rtv2,Temptv2,~,P2,RC2,H2,WPINm2,FPINm2,~,~,V1,V2,GNZI,GV,maxtau,~,CI] = LoadYemenData; % Load the data used to construct the figure
WI2=WI2(GNZI,:);
tA2=tA2(GNZI,:);
Rtv2=Rtv2(GNZI,:);
P2=P2(GNZI,:);
...
plot([1:247],TestH,'color',hex2rgb('#FAAF08'),'LineWidth',2); hold on; 
plot([1:247],TestG,'color',hex2rgb('#FAAF08'),'LineWidth',2,'LineStyle','-.'); hold on; 


figure('units','normalized','outerposition',[0 0 1 1]);
subplot('Position',[0.047268907563025,0.624113475177305,0.942752100840336,0.365886524822696]); 

plot([120:247],TestH(120:247),'color',hex2rgb('#FAAF08'),'LineWidth',2); hold on; 
plot([120:247],TestG(120:247),'color',hex2rgb('#FAAF08'),'LineWidth',2,'LineStyle','-.'); hold on; 

plot(184.*ones(101,1),linspace(200,450,101),'k:','LineWidth',2);
...
subplot('Position',[0.017268907563025,0.08,0.45,0.45]); % Creates a sub-panel to plot the figure in the x position is 0.0708 the y position is 0.163120567375887, 0.897162184873949 is the width and 0.793313069908819 is the heigt
MAR=mean(Wheatt2(:,[120:183]),2);
MARo=mean(Wheatt2(:,[184:247]),2);
MART=MAR(MAR>0);
MAR=(MAR-min(min(MARo(MARo>0)),min(MAR(MAR>0))))./(max(max(MARo),max(MAR))-min(min(MARo(MARo>0)),min(MAR(MAR>0))));
for ii=1:length(S)
    if(ii~=21)        
...
text(mean(xlim),10.78,'Average wheat price','Fontsize',16,'HorizontalAlignment','center');
ylim([11.7   19.0978]);
...
subplot('Position',[0.565,0.08,0.45,0.45]); % Creates a sub-panel to plot the figure in the x position is 0.0708 the y position is 0.163120567375887, 0.897162184873949 is the width and 0.793313069908819 is the heigt
MAR=mean(Wheatt2(:,[184:247]),2);
MARo=mean(Wheatt2(:,[120:183]),2);
MART=MAR(MAR>0);
MAR=(MAR-min(min(MARo(MARo>0)),min(MAR(MAR>0))))./(max(max(MARo),max(MAR))-min(min(MARo(MARo>0)),min(MAR(MAR>0))));
for ii=1:length(S)
    if(ii~=21)        
...
print(gcf,['Figure1_Wheat-COVID-19.png'],'-dpng','-r600');