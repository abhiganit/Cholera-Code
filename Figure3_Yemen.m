...
for mm=1:1
    ...
    hold on;
    for ii=1:3
        plot((maxtau+mean([IndW(ii,2) IndW(ii+1,1)])).*ones(1001,1),linspace(0,(1.4*10^4),1001),'k-.','LineWidth',2);        
    end
    ...
    for wv=1:4
        for jj=1:length(squeeze(WWRC(1,1,:)))
            ...
            patch([xstart xstart xend xend],[max(ylim) 7250./7000*(1.4*10^4) 7250./7000*(1.4*10^4) max(ylim)] ,ColorM(jj,:),'Edgealpha',0)
            if(round(100.*squeeze(WWRC(wv,mm,jj)))>=5)
            ht=text(mean([xstart xend]),mean([7250 8500]./7000*(1.4*10^4)),[num2str(round(100.*squeeze(WWRC(wv,mm,jj)))) '%'],'Fontsize',12,'HorizontalAlignment','center','color','w','Fontweight','bold');
            set(ht,'Rotation',90);
            end
        end
         xstart=maxtau+IndW(wv,1)+(IndW(wv,2)-IndW(wv,1)).*sum(squeeze(WWRC(wv,mm,1:(jj))));
         xend=maxtau+IndW(wv,1)+(IndW(wv,2)-IndW(wv,1));   
         patch([xstart xstart xend xend],[max(ylim) 7250./7000*(1.4*10^4) 7250./7000*(1.4*10^4) max(ylim)] ,[0.7 0.7 0.7],'Edgealpha',0)
         if(round(100.*(1-sum(squeeze(WWRC(wv,mm,1:(jj))))))>=5)
         ht=text(mean([xstart xend]),mean([7250 8500]./7000*(1.4*10^4)),[num2str(round(100.*(1-sum(squeeze(WWRC(wv,mm,1:(jj))))))) '%'],'Fontsize',12,'HorizontalAlignment','center','color','w','Fontweight','bold');
         set(ht,'Rotation',90);
         end
    end
