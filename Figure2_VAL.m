...
NW=length(WI(1,:));
bar([(1+maxtau):NW1],sum(MI(:,1:(NW1-maxtau))),'Facecolor',FC(1,:),'LineStyle','none','Facealpha',1); hold on
bar([(NW1+1):NW],sum(MI(:,(1+NW1-maxtau):end)),'Facecolor',FC(2,:),'LineStyle','none','Facealpha',1); hold on
...
for gg=1:5
    ...
    for ss=1:4        
        ...
        bar([(1+maxtau):NW1],(MI(GGG,1:(NW1-maxtau))),'Facecolor',FC(1,:),'LineStyle','none','Facealpha',1); hold on
        bar([(NW1+1):NW],(MI(GGG,(1+NW1-maxtau):end)),'Facecolor',FC(2,:),'LineStyle','none','Facealpha',1); hold on
        ...
        if((gg==5)&&(ss>2))
            if(rem(ss,2)==1)
                ...
                set(gca,'LineWidth',2,'tickdir','out','XTick',[1:dW:NW],'XTickLabel',XTL,'Fontsize',16,'Xminortick','on','YTick',[0:1000:8000],'YMinortick','on','YTickLabel','');
                xlabel('Week reported','Fontsize',18);
                hy=ylabel('','Fontsize',18);
            end
        else
            if(rem(ss,2)==1)
                set(gca,'LineWidth',2,'tickdir','out','XTick',[1:dW:NW],'XTickLabel','','Fontsize',16,'Xminortick','on','YTick',[0:1000:8000],'YMinortick','on');
                hy=ylabel('Suspected cholera cases','Fontsize',18);
            else                
                set(gca,'LineWidth',2,'tickdir','out','XTick',[1:dW:NW],'XTickLabel','','Fontsize',16,'Xminortick','on','YTick',[0:1000:8000],'YMinortick','on','YTickLabel','');
                hy=ylabel('','Fontsize',18);
            end
        end
        % Sets the y-axis to not have 10^n
        ...
        xtickangle(90);
        ylim([0 8000]);
        text(1.5,7700,Location{GGG},'Fontsize',18);
        text(hy.Extent(1),8750,char(65+GGG),'Fontsize',32,'FontWeight','bold');
    end
end
...
gg=6;
for ss=1:1        
    subplot('Position',[(0.065+0.535)/2,(0.6+0.19)/2,0.46,0.35]);
    GGG=ss+4.*(gg-1);    
    dW=10;
    XTL=datestr([startDateofSim+7.*[0:dW:(NW-1)]],'mm/dd/yy');
    NW=length(WI(1,:));
