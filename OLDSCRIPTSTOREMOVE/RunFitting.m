function RunFitting(XU,tau,AF,CF,RIF,RF,G,PF,PE,PP,DT)
...
if isfile([pwd '\Tables\ModelFitSummary.dat'])
     ...
     fprintf(fmf, 'Intercept \t Population \t HealthFacilities \t Incidence \t Attack_Incidence \t Conflcit_Incidence \t Rainfall_Incidence \t Precipitation \t External_Incidence \t Attack \t Attack_Function \t Conflict_Function \t Rainfall_Incidence_Function \t Precipitation_Function \t ObjectiveValue \t AIC \t MeanErrorProejection \t R2Projection \n'); % The header for the table
end
%% Load the data
...
% beta=[x(1) 10.^x(2) x(3) 10.^x(4:10)'].*XU;
% 
...
lb=[-16.*ones(1,17)]; % ensuring the lower bound is zero for the last nine paramters and do a log-10 transform to improve searching of paramter space
ub=[ log10([10^3  10^3 10^3 10 10 10 10 10^3 1 10^4 1 1 113 10 12 12 1])]; % specify the upperbound for the parameters 


%% Run the fitting algorithm
NS=1000; % The number of initial starting points used in the fitting process
opts= optimset('MaxIter',10^9,'MaxFunEvals',10^9,'TolFun',10^(-8),'TolX',10^(-8),'UseParallel',true,'Display','off');  % Specifies the conditions that will be used in the fitting processs
XS=RetStartpoint(XU,tau,AF,CF,RIF,RF,lb,ub);
problem = createOptimProblem('lsqnonlin','objective',@(x)OFuncResid(x,WI(GNZI,:),tA(GNZI,:),Ctv(GNZI,:),Rtv(GNZI,:),XU,tau,maxtau,AF,CF,RIF,RF,P(GNZI),H(GNZI)),'x0',XS,'lb',lb,'ub',ub,'Aineq',[],'bineq',[],'Aeq',[],'beq',[],'options',opts);
...
% Computing the statistical of the bets estimates for the regresison model
    ...
    [Yt,PDG,HFG,It,IAt,ICt,IRt,Rt,Gt,At]=LogisticModel(beta,WI(GNZI,:),tA(GNZI,:),DB,DA,DAE,Ctv(GNZI,:),K,n,Rtv(GNZI,:),RIF,rl,RF,rh,tau,maxtau,CF,P(GNZI),H(GNZI)); % Returns the incidence in matrix form of size Ng X (NW-tau)
    [resid]=OFuncResid(par,WI(GNZI,:),tA(GNZI,:),Ctv(GNZI,:),Rtv(GNZI,:),XU,tau,maxtau,AF,CF,RIF,RF,P(GNZI),H(GNZI)); % Returns the vector of residulas for each of the data points
    ...
    % Attacks affect environemntal facotrs
    if(XU(10)==1)
        X=[X At(:)]; % add product of past incidence and environmental factors
        twotail=[twotail 1]; % Specify whether want two tail or not
    end
    ...
    f1 = fopen([pwd '\Tables\Table-XU=' num2str(XU*((2.^[0:(length(XU)-1)])')) '-CF=' num2str(CF*XU(6)) '-AF=' num2str(AF*XU(5)) '-RIF=' num2str(RIF*XU(7)) '-PF=' num2str(RF*XU(8)) '-tau=' num2str(tau(XU(2:end)>0)) '.dat'],'w'); % Writing to the file name
    CN=struct('N',{'Intercept ','Population ','Health ','Incidence ','Attack_Incidence ','Conflcit ','Rainfall ','Precipitation ','External ','Attack '}); % The name of the variables that we can call
    ...
    if(DT~=0)
        readtable([pwd '\Tables\Table-XU=' num2str(XU*((2.^[0:(length(XU)-1)])')) '-CF=' num2str(CF*XU(6)) '-AF=' num2str(AF*XU(5)) '-RIF=' num2str(RIF*XU(7)) '-PF=' num2str(RF*XU(8)) '-tau=' num2str(tau(XU(2:end)>0)) '.dat']) % reads and prints the table to the command window
    end
    save([pwd '\Tables\Model-XU=' num2str(XU*((2.^[0:(length(XU)-1)])')) '-CF=' num2str(CF*XU(6)) '-AF=' num2str(AF*XU(5)) '-RIF=' num2str(RIF*XU(7)) '-PF=' num2str(RF*XU(8)) '-tau=' num2str(tau(XU(2:end)>0)) '.mat'],'k','beta','DB','DA','K','n','rl','rh','fval','tau','AIC','par'); % Saves the information for the specified model for can load and run model after
...
if(PE~=0)
    ...
    if(XU(5)~=0) % See if the function is present
        Y1 = ImpactAttack(testA,DB,DA,[0 0 0 0 0],0); % evalautae function the function for the days (do not need lag here as we are interested in the function)
    else
        Y1=0.*testA; % the function is absent so set to zero
    end
    if(XU(10)~=0) % See if the function is present
        Y2 = ImpactAttack(testA,0,DAE,[0 0 0 0 0],0); % evalautae function the function for the days (do not need lag here as we are interested in the function)
    else
...
if(PF~=0) % Plot fit 
    ...
    print(gcf,[pwd '\Figures\Fit-XU=' num2str(XU*((2.^[0:(length(XU)-1)])')) '-CF=' num2str(CF*XU(6)) '-AF=' num2str(AF*XU(5)) '-RIF=' num2str(RIF*XU(7)) '-PF=' num2str(RF*XU(8)) '-tau=' num2str(tau(XU(2:end)>0)) '.png'],'-dpng','-r600');
end
...
[Yt,Pt]= ModelProjection(beta,WI(GNZI,:),tA(GNZI,:),DB,DA,DAE,Ctv(GNZI,:),K,n,Rtv(GNZI,:),RIF,rl,RF,rh,tau,maxtau,CF,P(GNZI),H(GNZI),NW-length(WI(1,:))); % Run the projection
...
save([pwd '\Tables\Model-XU=' num2str(XU*((2.^[0:(length(XU)-1)])')) '-CF=' num2str(CF*XU(6)) '-AF=' num2str(AF*XU(5)) '-RIF=' num2str(RIF*XU(7)) '-PF=' num2str(RF*XU(8)) '-tau=' num2str(tau(XU(2:end)>0)) '.mat'],'RPro','RFit','MSEP','-append'); % Saves the information for the specified model for can load and run model after

% Save to model fit summary

fprintf(fmf, [ '%d \t %d \t  %d \t  %d \t  %d \t  %d \t  %d \t  %d \t  %d \t  %d \t ' AFF((AF+1).*XU(5)+1).N '\t ' CFF((CF+1).*XU(6)+1).N ' \t ' RFF((RIF+1).*XU(7)+1).N ' \t ' RFF((RF+1).*XU(8)+1).N ' \t %8.0f \t %5.2f \t %8.0f \t %3.2f \n'],[XU(1) XU(2:end).*tau fval AIC MSEP RPro]); % The header for the table
fclose(fmf);
...
if(PP~=0)
    ...
    print(gcf,[pwd '\Figures\FitandProjection-XU=' num2str(XU*((2.^[0:(length(XU)-1)])')) '-CF=' num2str(CF*XU(6)) '-AF=' num2str(AF*XU(5)) '-RIF=' num2str(RIF*XU(7)) '-PF=' num2str(RF*XU(8)) '-tau=' num2str(tau(XU(2:end)>0)) '.png'],'-dpng','-r600');
end
