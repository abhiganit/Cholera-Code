function [IndW,Wave_T,CCRT] = Contribution_to_Cholera_Incidence()
...
for kk=1:21
    ...
    for ww=1:4
        for mm=1:7
           WWRC(ww,1,mm) = sum(sum((CCR{kk,mm}(RC(GNZI)==1,IndW(ww,1):IndW(ww,2))),2))./sum(sum(MI(RC(GNZI)==1,IndW(ww,1):IndW(ww,2)),2));
           WWRC(ww,2,mm) = sum(sum((CCR{kk,mm}(RC(GNZI)==0,IndW(ww,1):IndW(ww,2))),2))./sum(sum(MI(RC(GNZI)==0,IndW(ww,1):IndW(ww,2)),2));
        end   
    end
    test_1=squeeze(WWRC(:,1,:));
    test_2=squeeze(WWRC(:,2,:));
    Wave_C{kk}=WWRC;
...
for mm=1:length(AICs)
    Wave_T=Wave_T+w_AIC(mm).*Wave_C{mm};
end
