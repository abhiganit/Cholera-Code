...
reshape([beta(XU==1)],4,5)'
reshape([pValue(XU==1)],4,5)'
% Conflict
...
betas=zeros(1,28);
[betap,fval,residual]=lsqnonlin(@(x)FitID(x,indx,betas,tA(GNZI,:),DB,DA,Ctv(GNZI,:),K,n,tau,maxtau,CF,WPIN(GNZI,:),FPIN(GNZI,:),Mt(GNZI,:),Wheatt(GNZI,:),Dieselt(GNZI,:),KP,V1(GNZI,:),V2(GNZI,:),KV,dV,Rtv(GNZI,:),RF,r0,WI(GNZI,:),PopS(GNZI,:),CI(GNZI,:),DAR,w),-6.*ones(size(indx)),-32.*ones(size(indx)),3.*ones(size(indx)));
betap=10.^betap;
betas=zeros(1,28);
betas(indx)=betap;
DiffMedA=[betas(5:12)]-[beta(5:12)]
[Yt,Xt]= LogisticModel(betas,tA(GNZI,:),DB,DA,Ctv(GNZI,:),K,n,tau,maxtau,CF,WPIN(GNZI,:),FPIN(GNZI,:),Mt(GNZI,:),Wheatt(GNZI,:),Dieselt(GNZI,:),KP,V1(GNZI,:),V2(GNZI,:),KV,dV,Rtv(GNZI,:),RF,r0,WI(GNZI,:),PopS(GNZI,:),CI(GNZI,:),DAR,w);
...
[bd,fval1]=fmincon(@(x)(mean((D-(10.^x(1)+10.^x(2).*C+10.^x(3).*S)).^2)),log10([162 32 1]),[],[],[],[],[-32 -32 -32],[3 3 3],[],opts);

RS=D-(10.^bd(1)+10.^bd(2).*C+10.^bd(3).*S);
temp=ones(size(C));
XCv=[temp(:) C S];
[SE,tStat,pValue] = LinRegressionStat(10.^bd,RS(:),3,XCv,0);
bd=10.^bd;
bd2=bd;
[sum(bd(2).*beta(13:16))]
