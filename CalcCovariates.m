...
% tau - the lag to use for the incidence and enviromental factors    
    %tau(3) - population density incidence
    % tau(4) - health zone incidence
    % tau(5) - Past incidence
    % tau(6) - Incidence other governorates
...
Gt=WPIN(:,(1+maxtau-tau(5)):(end-tau(5))).*P(:,(1+maxtau-tau(5)):(end-tau(5))).*WI(:,(1+maxtau-tau(5)):(end-tau(5))); 
HWt=H(:,(1+maxtau-tau(6)):(end-tau(6))).*WPIN(:,(1+maxtau-tau(6)):(end-tau(6))).*WI(:,(1+maxtau-tau(6)):(end-tau(6)));
RCt=repmat(RC,1,length(WI(1,(1+maxtau-tau(7)):(end-tau(7))))).*WI(:,(1+maxtau-tau(7)):(end-tau(7))); % Rebel control
% tau is estimated
ITAt=min(WI(:,(1+maxtau-tau(8)):(end-tau(8))),1).*WPIN(:,(1+maxtau-tau(8)):(end-tau(8))).*ImpactAttack(tA,DB(1),DA(1),tau(8),maxtau); % Product of incidence and attacks 
ICt=min(WI(:,(1+maxtau-tau(9)):(end-tau(9))),1).*WPIN(:,(1+maxtau-tau(9)):(end-tau(9))).*ImpactConflict(Ctv(:,(1+maxtau-tau(9)):(end-tau(9))),K(1),n(1),CF(1)); %Product of incidence and conflict
IAt=min(WI(:,(1+maxtau-tau(10)):(end-tau(10))),1).*WPIN(:,(1+maxtau-tau(10)):(end-tau(10))).*ImpactAttack(Mt,DBE(1),DAE(1),tau(10),maxtau); % Product of incidence and attacks 
IRt=WPIN(:,(1+maxtau-tau(11)):(end-tau(11))).*min(WI(:,(1+maxtau-tau(11)):(end-tau(11))),1).*ImpactRainfall(Rtv(:,(1+maxtau-tau(11)):(end-tau(11))),RIF(1),rl(1)); %Product of incidence and rainfall
Rt=WPIN(:,(1+maxtau-tau(12)):(end-tau(12))).*ImpactRainfall(Rtv(:,(1+maxtau-tau(12)):(end-tau(12))),RF(1),rh(1)); %rainfall
CRIt=ImpactConflict(Ctv(:,(1+maxtau-tau(13)):(end-tau(13))),K(2),n(2),CF(2)).*ImpactRainfall(Rtv(:,(1+maxtau-tau(13)):(end-tau(13))),RIF(2),rl(2)).*min(WI(:,(1+maxtau-tau(13)):(end-tau(13))),1); %Product of incidence and conflict
TARIt=ImpactAttack(tA,DB(2),DA(2),tau(14),maxtau).*ImpactRainfall(Rtv(:,(1+maxtau-tau(14)):(end-tau(14))),RIF(3),rl(3)).*min(WI(:,(1+maxtau-tau(14)):(end-tau(14))),1); % Product of incidence and attacks 
ARIt=ImpactAttack(Mt,DBE(2),DAE(2),tau(15),maxtau).*ImpactRainfall(Rtv(:,(1+maxtau-tau(15)):(end-tau(15))),RIF(4),rl(4)).*min(WI(:,(1+maxtau-tau(15)):(end-tau(15))),1); % Product of incidence and attacks 
