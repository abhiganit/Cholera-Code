function [par,fvalfit]=ProFittingPS(XU,CF,RF,pars)
...
options = optimoptions('ga','MaxGenerations',10^4,'FunctionTolerance',10^(-6),'InitialPopulationMatrix',parst,'Display','off','PlotFcn', @gaplotbestf,'HybridFcn',{@patternsearch,optionsps}); %
    [par,fvalfit] =patternsearch(@(x)OFuncProGA(x,CF,WI(GNZI,1:NW),tA(GNZI,1:NW),Ctv(GNZI,1:NW),XU,maxtau,WPIN(GNZI,1:NW),FPIN(GNZI,1:NW),Mt(GNZI,1:NW),Wheatt(GNZI,1:NW),Dieselt(GNZI,1:NW),V1(GNZI,1:NW),V2(GNZI,1:NW),Rtv(GNZI,1:NW),RF,PopS(GNZI,1:NW),CI(GNZI,1:NW)),parst,[],[],[],[],lb,ub,[],optionsps); 
