function [WI,Ctv,tA,Rtv,Temptv,Mt,P,RC,H,WPINm,FPINm,Dieselt,Wheatt,VT1,VT2,GNZI,GV,maxtau,PopS,CI] = LoadYemenDataVal
...
load('Yemen_Gov_Incidence_VAL.mat'); % Incidence data
CI=cumsum(IData',2);
...
Ctv=GLevelConflict(ProC,S,166);
load('Yemen_Air_Shelling.mat');
Mt=GLevelConflict(YASt,S,166); % Send conflict data (time, latitude, longatude) and shapefile of area wanted to catagorize
...
tA=GLevelConflict(ProA,S,166);% % Send attack data (time, latitude, longatude) and shapefile of area wanted to 
load('Precipitation_Gov_Project_Forward_VAL.mat'); % the perceipatiatino data for the differetn areas %Load the rainfall for the projection
...
W = shaperead([ pwd '/ShapeFile/WATERPRO_WORKING.shp']); % Shape file for Yemen
N = shaperead([ pwd '/ShapeFile/WATERPRO_NOTWORKING.shp']); % Shape file for Yemen7
S = shaperead([ pwd '/ShapeFile/yem_admbnda_adm1_govyem_mola_20181102.shp']); % Shape file for Yemen
tot=zeros(22,1);
for ii=1:22
inN = inpolygon([N.X]',[N.Y]',S(ii).X, S(ii).Y);
inW = inpolygon([W.X]',[W.Y]',S(ii).X, S(ii).Y);
tot(ii)=sum(inN)+sum(inW);
end

%tA=log(1+tA./repmat(A,1,length(tA(1,:))).*PopS);

%% Diesel and Wheat prices
load('Diesel_Gov_Yemen_VAL.mat')
load('Wheat_Gov_Yemen_VAL.mat')
