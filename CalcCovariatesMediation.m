function [X] = CalcCovariatesMediation(tA,DB,DA,Ctv,K,n,tau,maxtau,CF,WPIN,FPIN,Mt,Wheatt,betaD,KP,Rtv,RF,r0,WI,Pop,CI,DAR,w)
...
Dt=betaD(1)+betaD(2).*ImpactConflict(Ctv,K(1),CF(1));
% Targeted Attacks
for ii=1:maxtau
...
for ii=(3.*maxtau+1):4*maxtau
    X(ii,:,:)=(w.*WPIN(:,(1+maxtau-tau(ii)):(end-tau(ii)))+(1-w).*FPIN(:,(1+maxtau-tau(ii)):(end-tau(ii)))).*(Dt(:,(1+maxtau-tau(ii)):(end-tau(ii)))).*WI(:,(1+maxtau-tau(ii)):(end-tau(ii))).*Pop(:,(1+maxtau-tau(ii)):(end-tau(ii)))./(Pop(:,(1+maxtau-tau(ii)):(end-tau(ii)))+DAR.*CI(:,(1+maxtau-tau(ii)):(end-tau(ii))));
end
