...
for mm=1:length(ModelConflict)
    ...
    [bd,fval1]=ga(@(x)Mediation_Fitting(x,D,C_MA),length(XCF)+1,[],[],[],[],-6.*ones(length(XCF)+1,1),3.*ones(length(XCF)+1,1),[],opts);
    ...
    Boot_betaC=betaCv;
    Boot_betaC2=betaC2v;
    save(['Mediation_Analysis' CName{ModelUD{mm}}  '.mat'],'betaC','pValue_beta_Conflict','pValue_beta_Mediation','betaC2','betaCorr','pValue_Corr','C_MA','Boot_betaC','Boot_betaC2'); %
end