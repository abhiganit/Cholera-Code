...
PopS=[ repmat(AP(:,1),1,NW2016) repmat(AP(:,2),1,52)  repmat(AP(:,3),1,52)  repmat(AP(:,4),1,NW2019)]; % population size to feed into the IDPt calculation
 ...
 CCR=cell(7,1);
 MI=zeros(size(squeeze(X(1,:,:))));
 for mm=1:7
    
        tempmat=((1-EOVC).*(sum(beta((maxtau*(mm-1)+1):(mm.*maxtau))).*squeeze(X(maxtau*(mm-1)+1,:,:))))./WI(GNZI,maxtau:(end-1));
   
    tempmat(isnan(tempmat))=0;
tempmat(isinf(tempmat))=0;
    CCR{mm}=tempmat;
    MI=MI+tempmat./WI(GNZI,maxtau:(end-1));
 end



IndW=[1 21; 22 74; 75 121; 122 149]; % Index of wave for the data used in the regression model
WW=zeros(4,length(GNZI),7);
for ww=1:4
    for mm=1:7
...
ColorM=[0.5 0.5 0.5;
        [221,28,119]./255; % Targeted attacks
...
for mm=1:3
    ...
    Gint=Gintv(mm);
    b=bar([(1+maxtau):NW],[squeeze(CCR{7}(Gint,:));squeeze(CCR{1}(Gint,:)); squeeze(CCR{2}(Gint,:)); squeeze(CCR{3}(Gint,:)); squeeze(CCR{4}(Gint,:)); squeeze(CCR{5}(Gint,:)); squeeze(CCR{6}(Gint,:))]','Stacked','LineStyle','none');
    ...
    xlim([0.5 NW+0.5]);
     ylim([0 8500]./4250);
    text(NW+0.5,7000./4250,XGL(Gint),'Fontsize',16,'HorizontalAlignment','right');
    set(gca,'linewidth',2,'tickdir','out','XTick','','XTickLabel','','Fontsize',16,'XMinortick','off','YTick',[0:0.2:2]);
    box off;
    hold on;
    for ii=1:3
    ...
    for wv=1:4
        for jj=1:length(squeeze(WW(1,1,:)))-1
            if(jj==1)
            ...
            patch([xstart xstart xend xend],[max(ylim) 7250./4250 7250./4250 max(ylim)] ,ColorM(jj+1,:),'Edgealpha',0)
            if(round(100.*squeeze(WW(wv,Gint,jj)))>=5)
            ht=text(mean([xstart xend]),mean([7250 8500]./4250),[num2str(round(100.*squeeze(WW(wv,Gint,jj)))) '%'],'Fontsize',12,'HorizontalAlignment','center','color','w','Fontweight','bold');
            set(ht,'Rotation',90);
            end
        end
         xstart=maxtau+IndW(wv,1)+(IndW(wv,2)-IndW(wv,1)).*sum(squeeze(WW(wv,Gint,1:(jj))));
         xend=maxtau+IndW(wv,1)+(IndW(wv,2)-IndW(wv,1));   
         patch([xstart xstart xend xend],[max(ylim) 7250./4250 7250./4250 max(ylim)] ,[0.7 0.7 0.7],'Edgealpha',0)
         if(round(100.*(1-sum(squeeze(WW(wv,Gint,1:(jj))))))>=5)
         ht=text(mean([xstart xend]),mean([7250 8500]./4250),[num2str(round(100.*(1-sum(squeeze(WW(wv,Gint,1:(jj))))))) '%'],'Fontsize',12,'HorizontalAlignment','center','color','w','Fontweight','bold');
         set(ht,'Rotation',90);
         end
     end
end
...
for mm=4:6
    ...
    hold on;
    for ii=1:3
        plot((maxtau+mean([IndW(ii,2) IndW(ii+1,1)])).*ones(1001,1),linspace(0,7000./4250,1001),'k-.','LineWidth',2);
    end
...
set(gca,'linewidth',2,'tickdir','out','XTick',[1:dW:NW],'XTickLabel',XTL,'Fontsize',16,'XMinortick','on','YTick',[0:0.2:2]);
