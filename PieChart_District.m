...
endDateofSim = datenum('5-01-2017');% End date
TruncV=ceil((1+endDateofSim-startDateofSim)./7);
IndW=[TruncV 74; 75 121; 122 149]-(TruncV-1);% Index of wave for the data used in the regression model
...
explode = [0,1,1,0,0,0];
ColorM=[hex2rgb('#DE7A22'); %Conflict
...
for kk=1:4
    ...
    for nn=1:2
        for jj=1:3    
            ...
            colormap(ColorM)
            Temp1=XGL(jj+3.*(nn-1)+6.*(kk-1));
            if(jj+3.*(nn-1)+6.*(kk-1)<=21)
                Temp2=XDGL(jj+3.*(nn-1)+6.*(kk-1));
                title({[Temp1{1} ', ' Temp2{1}],[ '(' num2str(round(100.*(1-WW(jj+3.*(nn-1)+6.*(kk-1),7)),1)) '%)']},'Fontsize',18)
            else
                title({Temp1{1},[ '(' num2str(round(100.*(1-WW(jj+3.*(nn-1)+6.*(kk-1),7)),1)) '%)']},'Fontsize',18)
            end
        end
    end
