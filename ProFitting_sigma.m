function [par,fval]=ProFitting_sigma(XU,par)
...
[sigma_w,fval] =ga(@(x)OFuncPro_sigma(x,par,WI(GNZI,1:NW),tA(GNZI,1:NW),Ctv(GNZI,1:NW),XU,maxtau,WPIN(GNZI,1:NW),FPIN(GNZI,1:NW),Mt(GNZI,1:NW),Wheatt(GNZI,1:NW),Dieselt(GNZI,1:NW),V1(GNZI,1:NW),V2(GNZI,1:NW),Rtv(GNZI,1:NW),Temptv(GNZI,1:NW),PopS(GNZI,1:NW),CI(GNZI,1:NW)),1,[],[],[],[],-6,0,[],[],options); 
par(end)=sigma_w;
end