...
for mm=1:2
    subplot('Position',[0.06,0.14+(3-mm)*0.29,0.935/2,0.27]);
    ...
    set(gca,'linewidth',2,'tickdir','out','XTick','','XTickLabel','','Fontsize',16,'XMinortick','off','YminorTick','on','YTick',[0:500:3500]);
    box off;
    hold on;
    for ii=1:3
    ...
    for wv=1:4
     ...
     text(yh.Extent(1)*0.95,max(ylim)*0.975,char(67+2*mm-1),'Fontsize',32,'fontweight','bold');
end
...
for mm=3:4
    subplot('Position',[0.06,0.14+(3-(mm-2))*0.29,0.935/2,0.27]);
    ...
    for wv=1:4
     ...
     text(yh.Extent(1)*0.95,max(ylim)*0.975,char(67+2*(mm-2)),'Fontsize',32,'fontweight','bold');
end
set(gca,'linewidth',2,'tickdir','out','XTick',[1:dW:NW],'XTickLabel',XTL,'Fontsize',16,'XMinortick','on','YTick',[0:500:3500]);
xlabel('Week of report','Fontsize',18);
xtickangle(45);
