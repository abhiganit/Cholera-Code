...
for mm=1:3
    ...
    set(gca,'linewidth',2,'tickdir','out','XTick','','XTickLabel','','Fontsize',16,'XMinortick','off','YTick',[0:0.2:1]);
    box off;
    hold on;
    for ii=1:3
        plot((maxtau+mean([IndW(ii,2) IndW(ii+1,1)])).*ones(1001,1),linspace(0,1,1001),'k-.','LineWidth',2);        
    end
...
%         b(ii).CData = ColorM(ii,:);
%     end
%     ylabel('Relative contribution','Fontsize',18);
%     xlim([0.5 NW+0.5]);
%      ylim([0 1.05]);
%     text(NW+0.5,1,XGL(Gint),'Fontsize',16,'HorizontalAlignment','right');
%     set(gca,'linewidth',2,'tickdir','out','XTick','','XTickLabel','','Fontsize',16,'XMinortick','off','YTick',[0:0.2:1]);
...
%     
% end
set(gca,'linewidth',2,'tickdir','out','XTick',[1:dW:NW],'XTickLabel',XTL,'Fontsize',16,'XMinortick','on','YTick',[0:0.2:1]);
