...
% Test convergence
 for cvii=1:11
     PDS=0.4+0.05.*(cvii-1)
    load(['ForwardSelectionNoRainNoConflict-Vaccination-alpha=0-PercentData=' num2str(PDS*100) '.mat']);
    IndxAllow=[1:6];
    gg=find(XUv(end,IndxAllow)==0);
    IndxAllow=IndxAllow(gg);
    atest=0;
    ...
    save(['ForwardSelectionNoRainNoConflict-Vaccination-alpha=0-PercentData=' num2str(PDS*100) '.mat']);
    while(~isempty(XUr))
        par=[partemp];
        RSSv=[RSSv; RSSr];
        CVE=[CVE; CVEr];
