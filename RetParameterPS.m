function [k,beta,tau,DB,DA,DBE,DAE,K,n,rl,rh,CF,RIF,RF]=RetParameterPS(x,XU)
...
nob=length(XU);
tau=[1 1 1 1 1 1 1 ceil(4.*x(nob+[1:8]))];
CF=ceil(3.*x(nob+[9:10]))-1;
RF=ceil(3.*x(nob+11))-1;
RIF=ceil(3.*x(nob+[12 13 14 15]))-1;
lenbeta=length(XU)+13;
